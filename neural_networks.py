...
class linear_layer:
    ...
    def __init__(self, input_D, output_D):
        ...
        self.W_shape = (input_D,output_D)
        self.b_shape = (1,output_D)

        self.params['W'] = np.random.normal(0,0.1,self.W_shape)
        self.params['b'] = np.random.normal(0,0.1,self.b_shape)
        ...
        self.gradient['W'] = np.zeros(self.W_shape)
        self.gradient['b'] = np.zeros(self.b_shape)
...
class tanh:
    ...
    def backward(self, X, grad):
        ...
        backward_output = np.multiply(grad,1 - np.square(np.tanh(X)))
        return backward_output
